...
public class NewBeeMallGoodsAPI {
    ...
    @RequestMapping(value = "/getAllShopGoods",method = RequestMethod.GET)
    public Result getAllGoods(){
        List<Object> res=new ArrayList<>();
        List<NewBeeMallGoods> allGoods = newBeeMallGoodsMapper.getAllShopGoods();
        for (NewBeeMallGoods goods:allGoods){
            Long user = goods.getCreateUser();
            Shop shop = shopService.getShopInfoById(user);
            Map<String,Object> item =new HashMap<>();
            item.put("goodsId",goods.getGoodsId());
            item.put("goodsCoverImg",goods.getGoodsCoverImg());
            item.put("goodsIntro",goods.getGoodsIntro());
            item.put("goodsName",goods.getGoodsName());
            item.put("sellingPrice",goods.getSellingPrice());
            if(ObjectUtils.isEmpty(shop)){
                item.put("shopId","1639652002534998018");
            }else{
                item.put("shopId",shop.getId());
            }
            item.put("tag",goods.getTag());
                    res.add(item);
        }
